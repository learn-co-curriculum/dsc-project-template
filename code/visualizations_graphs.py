...
def time_series_roll_plot(dataset,title,xlab,ylab,file_name, color, y_tick_num):
        """This function:
        - takes a pd.Series output from value_counts() and converts it to a pd.DataFrame
        - formats the css and html to make it look nice
        - renders the html
        ...
        color=color
        lin_style = ['-', '--', ':']
        cm.rainbow(np.linspace(0,1,len(data.columns.tolist())))

        # Loops through species catagory and adds them to the plot
        for col_name, l in zip(data.columns.tolist(),lin_style):
            ax.plot(data.index, col_name, linestyle=l, data=data, linewidth=4,c=color)
